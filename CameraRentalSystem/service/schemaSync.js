...
async function migrateUsersIfExist(hasUsers) {
  if (!hasUsers) return;
...
async function migrateCamerasIfExist(hasCameras) {
...
function getBookingCustomerJoinSql(hasUsers) {
  return hasUsers
...
async function migrateBookingsIfExist(hasBookings, hasUsers) {
  if (!hasBookings) return;
  const joinSql = getBookingCustomerJoinSql(hasUsers);
...
async function migratePaymentsIfExist(hasPayments, hasBookings, hasUsers) {
  if (!hasPayments || !hasBookings) return;
  const joinSql = getBookingCustomerJoinSql(hasUsers);
...
async function migrateLegacyPostgresTables() {
  ...
  await migrateUsersIfExist(hasUsers);
  await migrateCamerasIfExist(hasCameras);
  await migrateBookingsIfExist(hasBookings, hasUsers);
  await migratePaymentsIfExist(hasPayments, hasBookings, hasUsers);
}
